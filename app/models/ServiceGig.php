...
class ServiceGig extends Model
{
    ...
    {
        $this->db->query("SELECT gigid, tu.tuid, title, price, revisions, duration, subject, image, rating, jobs, level, username, firstname, lastname, photourl  FROM api.servicegig sg, api.tutor tu, api.user where sg.tuid = tu.tuid and tu.userid = user.userid where gigid = :gigid;");
